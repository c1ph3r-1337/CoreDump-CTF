...
const targetStr = `if (data.error) {
            profileDiv.innerHTML = \`
              <div id="displayUsernameContainer"`;

const startIdx = html.indexOf('if (data.error) {');
if (startIdx > -1) {
    const endStr = `            \`;\n\n            document.getElementById('editUsernameBtn')`;
    const endIdx = html.indexOf(endStr, startIdx);
    
    if (endIdx > -1) {
        const fullBlock = html.substring(startIdx, endIdx);
        const fixedBlock = `if (data.error) {
...
`;
          html = html.substring(0, startIdx) + fixedBlock + html.substring(endIdx);
