...
const errorIfIndex = lines.findIndex(l => l.includes('if (data.error) {'));

if (errorIfIndex !== -1) {
    // Delete from errorIfIndex to the end of the fetchProfile function
    const catchIndex = lines.findIndex((l, i) => i > errorIfIndex && l.includes('.catch(err => console.error(\'Error fetching profile:\', err));'));
    
    if (catchIndex !== -1) {
        const newBlock = `          if (data.error) {
        ...
        lines.splice(errorIfIndex, catchIndex - errorIfIndex + 1, newBlock);
        fs.writeFileSync('private/dashboard.html', lines.join('\n'));
        console.log('Fixed lines!');
    } else {
        console.log('Catch index not found');
    }
}
